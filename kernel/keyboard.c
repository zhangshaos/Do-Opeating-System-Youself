...
PUBLIC void keyboard_handler(int irq)
{
	...
	if (kb_in.count < KB_IN_BYTES)
	{
		*(kb_in.p_head) = scan_code;
		...
		/* p_head 指向下一个空闲的区域 */
		kb_in.p_head = kb_in.buf + (kb_in.p_head - kb_in.buf + 1)%KB_IN_BYTES;
...
/*======================================================================*
                           keyboard_read
	================================================================
				从kb_in[]中读取扫描码,并调用respond_key()响应按键
*======================================================================*/
PUBLIC void keyboard_read(TTY* p_tty)
{
	...
	u32		key = 0;/* 用一个整型来表示一个键。比如，如果 Home 被按下，则 key 值将为定义在 keyboard.h 中的 'HOME'. */
	...
	if(kb_in.count > 0)
	{
		...
		/* 下面开始解析扫描码,先判断两个扫描码-make长度>2的特殊按键 */
		if (scan_code == 0xE1) 
		{
...
PRIVATE u8 get_byte_from_kbuf()
{
	...
	/* p_tail 指向下一个待读取的扫描码 */
	kb_in.p_tail = kb_in.buf + (kb_in.p_tail - kb_in.buf + 1)%KB_IN_BYTES;
    kb_in.count--;
...
/*======================================================================*
				 			set_leds
	================================================================
		设置caps_lock & num_lock & scroll_lock锁定键的led灯
 *======================================================================*/
PRIVATE void set_leds()
{
