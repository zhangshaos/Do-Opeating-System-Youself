 ...
 * 包含的内部消息
 */
#define	DIOCTL_GET_GEO	1   /* 获取请求设备的起始扇区和扇区数目 */
...
#define	MAKE_DEV(a,b)	((a << MAJOR_SHIFT) | b)    /* 主设备号 & 次设备号 */

 /* separate major and minor numbers from device number */
#define	MAJOR(x)		((x >> MAJOR_SHIFT) & 0xFF) /* 获得主设备号 */
#define	MINOR(x)		(x & 0xFF)                  /* 获得次设备号 */

 /* device numbers of hard disk */
#define	MINOR_hd1a		0x10    /* 定义扩展分区,第一个逻辑分区的次设备号 */
...
/**@Warning
 * I adopt the different means to encode hard disk's minor number
 * the number in backet is minor device number.
 * |                      hd0(0)                    |(代表整块硬盘)
...
/**@ Command Block Registers
 * I/O msg to hd's command block registers
 */
struct hd_cmd 
{
	u8	features;
